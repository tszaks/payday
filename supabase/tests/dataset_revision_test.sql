...
-- One statement, three rows, ONE bump. Row-level triggers would give three,
-- and `import_shifts` moves rows in bulk.
insert into public.shifts (id, user_id, work_date, cash_tips_cents, client_updated_at)
...
select pg_temp.expect('theWatermarkCoversEveryMoneyBearingTable',
  (select count(*) from pg_trigger t join pg_class c on c.oid = t.tgrelid
    where c.relname in ('shifts','paycheck_records','user_settings')
      and t.tgname like '%_bump_revision_%' and not t.tgisinternal) = 9,
  'triggers=' || (select count(*) from pg_trigger t join pg_class c on c.oid = t.tgrelid
    where c.relname in ('shifts','paycheck_records','user_settings')
      and t.tgname like '%_bump_revision_%' and not t.tgisinternal));
...
select pg_temp.expect('theBumpIsNotCallableByAClient',
  not has_function_privilege('authenticated', 'private.bump_dataset_revision(uuid[])', 'execute')
  and not has_function_privilege('anon', 'private.bump_dataset_revision(uuid[])', 'execute'));
