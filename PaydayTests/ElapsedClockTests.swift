...
struct ElapsedClockTests {
    ...
    func underAnHour() {
        #expect(ElapsedClock.string(from: start, to: start.addingTimeInterval(0)) == "00:00")
        #expect(ElapsedClock.string(from: start, to: start.addingTimeInterval(7)) == "00:07")
        #expect(ElapsedClock.string(from: start, to: start.addingTimeInterval(47 * 60 + 5)) == "47:05")
        #expect(ElapsedClock.string(from: start, to: start.addingTimeInterval(3599)) == "59:59")
    }

    @Test("from the hour mark reads H:MM:SS")
    func hourAndBeyond() {
    ...
    func negativeClampsToZero() {
        #expect(ElapsedClock.string(from: start, to: start.addingTimeInterval(-30)) == "00:00")
    }
}
