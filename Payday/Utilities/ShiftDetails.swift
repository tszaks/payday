...
enum ShiftDetails {
    /// The canonical hours/tip-out/sales for a set of entries covering one
    /// night, resolved from whichever entry actually holds it — credit
    /// first, falling back to cash — and NEVER summed across entries.
    static func resolve(from entries: [TipEntry]) -> (hoursWorked: Double?, tipOutCents: Int?, salesCents: Int?) {
    ...
    /// Writes hours/tip-out/sales onto the one canonical entry for a night
    /// (credit when one exists, else the first entry) and clears them from
    /// every other entry sharing that night — self-healing any data where
    /// a value ended up set on more than one entry.
    static func write(hoursWorked: Double?, tipOutCents: Int?, salesCents: Int?, into entries: [TipEntry]) {
